...
def get_repetition_rate(Tensor_1, Tensor_2):
    # 将Tensor转换为集合
    set1 = set(Tensor_1.tolist())
    set2 = set(Tensor_2.tolist())

    intersection = set1.intersection(set2)

    num_common_elements = len(intersection)

    min_length = min(Tensor_1.numel(), Tensor_2.numel())

    repetition_rate = num_common_elements / min_length

    return repetition_rate

# import torch
#
# a=torch.tensor([42, 43, 44, 50, 51, 52, 58, 59, 60])
# b=torch.tensor([42, 43, 44, 50, 51, 52, 58, 59, 60])
# print(get_repetition_rate(a,b))