...
class Align(nn.Module):
    def __init__(self, c_in, c_out, kt=1):
        ...
        self.pool = nn.MaxPool2d(kernel_size=(kt, 1), stride=(kt, 1))  # 池化层
...
class TemporalConvLayer(nn.Module):
    def __init__(self, kt, c_in, c_out, act="relu"):
        ...
        self.act = act
        self.c_out = c_out
        self.align = Align(c_in, c_out, kt)
        if self.act == "GLU":
            '''
                新加
            '''
            if self.kt == 11 or self.kt==19:
                # print(1)
                self.conv = nn.Conv2d(c_in, c_out * 2, (self.kt, 1), 1, padding=[0, 0])
            else:
...
class STGCN(nn.Module):
    def __init__(self, args_predictor, device, dim_in=2, dim_out=2):
