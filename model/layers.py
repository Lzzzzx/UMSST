...
class STEncoder(nn.Module):
    def __init__(self, Kt, Ks, blocks, input_length, num_nodes, batch_size, attenuation='log', attenuation_rate=1,
                 droprate=0.1):
        super(STEncoder, self).__init__()
        self.Ks = Ks
        self.batch_size = batch_size
        self.attenuation = attenuation  # attenuation function
        self.attenuation_rate = attenuation_rate  # attenuation function weight
        ...
        '''
            需要修改out_len的结果维度 然后把这一部分弄出来作为后续输入embedding
        '''
        ...
        self.receptive_field = input_length + Kt - 1  # 感受野 感受野为什么一定要设置为这么大呢
    ...
    def forward(self, x0, graph):
        ...
        # lib.attenuation.Attenuation.func_attenuation_1()


        # print(in_len)
        if in_len < self.receptive_field:
            # F.pad()填充函数 填充的顺序 左右上下 后面就是三维填充 上下
        ...
        # print(f"第一次卷积后x.size={x.size()}")
        x, x_agg, self.t_sim_mx = self.pooler(x)
        # print(f"x_agg.size={x_agg.size()},{x.size()}")
        self.s_sim_mx = sim_global(x_agg, sim_type='cos')
        # print(f"s_sim_mx.size={self.s_sim_mx.size()}")
        ...
        # print(x.size())
        '''这个x_downstream将作为下游任务的输入'''
        x_downstream = self.dropout3(x.permute(0, 2, 3, 1))
        ...
        # print(f"最后的x.size={x.size()}")
        return x, x_downstream  # nl(=1)vc , nl(=input_length - 2 * (Kt - 1) * len(blocks))vc
...
class Pooler(nn.Module):
    ...
    def __init__(self, n_query, d_model, agg='avg'):
        ...
        self.softmax = nn.Softmax(dim=2)  # softmax on the seq_length dim, nclv   in and out flow
